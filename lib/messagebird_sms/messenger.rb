  ...
  class Messenger
    ...
    def self.method_missing(method_name, *args) # :nodoc:
      if new.respond_to?(method_name.to_s)
        MessagebirdSms::MessageDelivery.new(self, method_name, *args)
      else
        super
      end
    end
    ...
    def self.default_params
      @@default_params ||= MessagebirdSms.config.defaults
    end
    ...
    def self.default(hash = {})
      self.default_params = MessagebirdSms.config.defaults.merge(hash).freeze
      default_params
    end
